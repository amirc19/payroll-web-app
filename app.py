    ...
    try:
        ...
        for driver_name, occurrences in driver_occurrences.items():
            ...
            if len(occurrences) == 1:
                ...
                # Extract data based on column position with better error handling
                if name_col == 3 and len(row) > 26:
                    total_stops = 0
                    if len(row) > 9 and row[9] and str(row[9]).replace('.0', '').replace('.', '').isdigit():
                        total_stops += int(float(row[9]))
                    if len(row) > 11 and row[11] and str(row[11]).replace('.0', '').replace('.', '').isdigit():
                        total_stops += int(float(row[11]))
                    ...
                    if len(row) > 8 and row[8] and str(row[8]).replace('.0', '').replace('.', '').isdigit():
                        total_stops += int(float(row[8]))
                    if len(row) > 10 and row[10] and str(row[10]).replace('.0', '').replace('.', '').isdigit():
                        total_stops += int(float(row[10]))
